...
/**
 * Suggests complete phrases through Wayfinder's terms component.
 *
...
class Suggester extends SuggesterPluginBase implements PluginFormInterface {
  ...
  public function getAutocompleteSuggestions(QueryInterface $query, $incomplete_key, $user_input) {
    ...
    $langcode = $config['drupal/langcode'] ?? 'any';
    if ($langcode !== 'any' && $langcode !== 'multilingual') {
      $query->addCondition('search_api_language', $langcode);
    }

    return $backend->getAutocompleteSuggestions(
      $query,
      $this->getSearch(),
